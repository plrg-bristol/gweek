...
//! frames, and the logic and suspension environments; the stack is a persistent arena cons-list,
//! so cloning it is one pointer copy. The scheduler never calls `step` directly — it calls
//! [`run_to_branch`](Machine::run_to_branch), which runs `step` tight and yields a [`RunResult`] at
//! the next branch, at completion, or on timeout. Each `step` matches on the head computation —
//! sequencing, functions, the functional-logic forms, the eliminators (which *case-split* on an
//! unbound logic variable), and `Rec` — and reads the deadline through a `Clock` only once every
//! 1024 ticks, lest a divergent loop never look at it.
