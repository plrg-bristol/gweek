...
The fair strategy maintains a queue of active machines. It dequeues the front
machine and steps it in a \DFS{}-like manner for a fixed quota (currently
$10{,}000$ steps). If the machine has not terminated or fully branched within
the quota, it is re-enqueued together with any unexplored branches.

This strategy is both complete and fair: every branch receives a bounded share
of computation, preventing any single branch from starving others. It is
particularly effective for programs that mix finite branching (explicit
